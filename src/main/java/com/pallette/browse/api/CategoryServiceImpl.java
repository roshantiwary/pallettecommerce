...
public class CategoryServiceImpl implements CategoryService{
	...
	public CategoryResponseBean getCategoryById(String categoryId) throws IllegalAccessException, InvocationTargetException {
		...
		CategoryResponseBean response = null;
		 ...
		 CategoryDocument category = categoryRepository.findOne(categoryId);
		if (null != category) {
			logger.info(String.format("CategoryServiceImpl.getCategoryById - retrieved Category is: " , category.getId()));
			response = browseServices.constructCategoryResponse(new ArrayList<CategoryDocument>(Arrays.asList(category)));
		} else {
