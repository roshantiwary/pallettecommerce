...
public class ProductServiceImpl implements ProductService {
	...
	public ProductResponseBean getProductById(String productId) throws IllegalAccessException, InvocationTargetException {
		...
		ProductDocument product = productRepository.findOne(productId);
		if (null != product) {
			logger.info(String.format("ProductServiceImpl.getProductById - retrieved product is: " , product.getId()));
			response = browseServices.constructProductResponse(new ArrayList<ProductDocument>(Arrays.asList(product)));
		} else {
